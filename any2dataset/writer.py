""""writer module handle writing the files to disk"""
...
class ParquetSampleWriter:
    """ParquetSampleWriter is a file+caption writer to parquet"""
    ...
    def write(self, file_str, key, caption, meta):
        """Keep sample in memory then write to disk when close() is called"""
        if file_str is not None:
            sample = {"key": key, "flac": file_str}
...
class WebDatasetSampleWriter:
    """WebDatasetSampleWriter is a file+caption writer to webdataset"""
    ...
    def write(self, file_str, key, caption, meta):
        ...
        if file_str is not None:
            sample = {"__key__": key, "flac": file_str}
            if self.save_caption:
                sample["txt"] = str(caption) if caption is not None else ""
...
class TFRecordSampleWriter:
    """TFRecordSampleWriter is a file+caption writer to TFRecord"""
    ...
    def write(self, file_str, key, caption, meta):
        ...
        if file_str is not None:
            sample = {
                ...
                "flac": self._bytes_feature(file_str),
            }
            if self.save_caption:
...
class FilesSampleWriter:
    """FilesSampleWriter is a caption+file writer to files"""
